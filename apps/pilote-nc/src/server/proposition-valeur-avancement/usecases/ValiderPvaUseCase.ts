...
export default class ValiderPvaUseCase {
  constructor(
  ...
  async run(input: {
    ...
    return this.dependencies.transaction.run(async () => {
      ...
      const tauxRealisation = calculerTauxRealisation({
        ...
        valeurActuelle: pva.valeurProposee,
      ...
      const pvaDecidee = await this.dependencies.pvaRepository.décider({
        id: pva.id,
        statut: StatutPva.VALIDEE,
        valeurValidee: pva.valeurProposee,
        motifRefus: null,
      ...
      await this.dependencies.pvaRepository.ajouterEvenement({
        pvaId: pva.id,
        type: TypeEvenementPva.VALIDEE,
        valeur: pva.valeurProposee,
